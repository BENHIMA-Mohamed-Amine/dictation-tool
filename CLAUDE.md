...
├── transcript_window.py       # TranscriptWindow — GTK window: editable transcript + Start/Stop, Copy, Clear buttons. Shown at launch; the Start/Stop button here is the everyday control (tray menus close on every click). One GTK mark splits settled text (user-editable) from the live partial tail (the only part rewritten); a second mark tracks where the current recording started. The buffer's `changed` signal is the single source of truth for clipboard sync — it fires on every dictated update *and* every manual edit, and always re-copies from the segment-start mark to end (this recording's text only, not the whole accumulated buffer), so the clipboard can never drift from what's on screen. We deliberately don't block Stop on the provider's post-stop finalize/correction round-trip (speed over trailing-word accuracy, by choice) — whatever partial was last shown gets copied as-is. The Copy button still copies the full accumulated buffer. **Known limitation (accepted, not a bug):** on this user's Wayland/GNOME (mutter) session, the compositor only lets the *focused* client claim the clipboard selection — so background auto-copy silently fails to update the clipboard while another window has focus, even though the sync logic itself runs correctly. The window must be focused/clicked right before pasting for the latest text to actually be there. A live-typing alternative (ydotool, unlike the xdotool tried in plans/done/08-remove-auto-output.md which can't see Wayland windows at all) would sidestep this but wasn't chosen — user opted to keep clipboard and accept the limitation
├── main.py                    # App orchestrator — wires ConfigStore, DictationController, TrayIcon, TranscriptWindow, SettingsWindow, ControlSocket; runs Gtk.main(). ControlSocket listens on ~/.config/dictation-tool/ctl.sock for start/stop/quit messages sent by dictation_ctl.py (a GNOME custom keyboard shortcut), reusing the same _on_toggle path the tray click uses — see plans/13-hotkey-control.md (an earlier single-message `toggle` version of this was tried and removed for a GNOME-binding issue since root-caused, see plans/done/10-remove-global-hotkey.md)
├── dictation_ctl.py           # `dictation_ctl.py start|stop|quit` — sends a control message to the running app's ControlSocket; `start` launches the app via `dictation start` first if it's not running. Meant to be bound to a GNOME custom keyboard shortcut, not run interactively
